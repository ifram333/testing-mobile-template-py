...
class CheckoutAddressPage(BasePage):
    ...
    def click_to_payment_button(self):
        ...
        print('Click "TO PAYMENT" button')

    # Send Keys methods
    def send_keys_full_name_input(self, full_name):
        self.send_keys(full_name, *self.txt_full_name)
        print('Fill "FULL NAME" with value ' + full_name)

    def send_keys_address_line_1_input(self, address_line_1):
        self.send_keys(address_line_1, *self.txt_address_line_1)
        print('Fill "ADDRESS LINE 1" with value ' + address_line_1)

    def send_keys_address_line_2_input(self, address_line_2):
        self.send_keys(address_line_2, *self.txt_address_line_2)
        print('Fill "ADDRESS LINE 2" with value ' + address_line_2)

    def send_keys_city_input(self, city):
        self.send_keys(city, *self.txt_city)
        print('Fill "CITY" with value ' + city)

    def send_keys_state_region_input(self, state_region):
        self.send_keys(state_region, *self.txt_state_region)
        print('Fill "STATE/REGION" with value ' + state_region)

    def send_keys_zip_code_input(self, zip_code):
        self.send_keys(zip_code, *self.txt_zip_code)
        print('Fill "ZIP CODE" with value ' + zip_code)

    def send_keys_country_input(self, country):
        self.send_keys(country, *self.txt_country)
        print('Fill "COUNTRY" with value ' + country)

    # Validation methods
    def validate_page(self):
        assert self.is_element_found(*self.lbl_checkout) is True
        assert self.is_element_value_equal_to('Checkout', *self.lbl_checkout) is True
        assert self.is_element_found(*self.lbl_enter_a_shipping_address) is True
        assert self.is_element_value_equal_to('Enter a shipping address', *self.lbl_enter_a_shipping_address) is True
        assert self.is_element_found(*self.lbl_full_name) is True
        assert self.is_element_value_equal_to('Full Name*', *self.lbl_full_name) is True
        assert self.is_element_found(*self.txt_full_name) is True
        assert self.is_element_found(*self.lbl_address_line_1) is True
        assert self.is_element_value_equal_to('Address Line 1*', *self.lbl_address_line_1) is True
        assert self.is_element_found(*self.txt_address_line_1) is True
        assert self.is_element_found(*self.lbl_address_line_2) is True
        assert self.is_element_value_equal_to('Address Line 2', *self.lbl_address_line_2) is True
        assert self.is_element_found(*self.txt_address_line_2) is True
        assert self.is_element_found(*self.lbl_city) is True
        assert self.is_element_value_equal_to('City*', *self.lbl_city) is True
        assert self.is_element_found(*self.txt_city) is True
        assert self.is_element_found(*self.lbl_state_region) is True
        assert self.is_element_value_equal_to('State/Region', *self.lbl_state_region) is True
        assert self.is_element_found(*self.txt_state_region) is True
        assert self.is_element_found(*self.lbl_zip_code) is True
        assert self.is_element_value_equal_to('Zip Code*', *self.lbl_zip_code) is True
        assert self.is_element_found(*self.txt_zip_code) is True
        assert self.is_element_found(*self.lbl_country) is True
        assert self.is_element_value_equal_to('Country*', *self.lbl_country) is True
        assert self.is_element_found(*self.txt_country) is True
        print('The Checkout page is loaded correctly')

    def validate_full_name_error_message(self):
        assert self.is_element_found(*self.lbl_full_name_error_message) is True
        assert self.is_element_value_equal_to('Please provide your full name.', *self.lbl_full_name_error_message) is True
        print('The correct full name error message (Please provide your full name.) is displayed')

    def validate_address_line_1_error_message(self):
        assert self.is_element_found(*self.lbl_address_line_1_error_message) is True
        assert self.is_element_value_equal_to('Please provide your address.', *self.lbl_address_line_1_error_message) is True
        print('The correct address line 1 error message (Please provide your address.) is displayed')

    def validate_address_line_2_error_message(self):
        assert self.is_element_found(*self.lbl_address_line_2_error_message) is True
        assert self.is_element_value_equal_to('Please provide your address.', *self.lbl_address_line_2_error_message) is True
        print('The correct address line 2 error message (Please provide your address.) is displayed')

    def validate_city_error_message(self):
        assert self.is_element_found(*self.lbl_city_error_message) is True
        assert self.is_element_value_equal_to('Please provide your city.', *self.lbl_city_error_message) is True
        print('The correct city error message (Please provide your city.) is displayed')

    def validate_state_region_error_message(self):
        assert self.is_element_found(*self.lbl_state_region_error_message) is True
        assert self.is_element_value_equal_to('Please provide your state/region.', *self.lbl_state_region_error_message) is True
        print('The correct state/region error message (Please provide your state/region.) is displayed')

    def validate_zip_code_error_message(self):
        assert self.is_element_found(*self.lbl_zip_code_error_message) is True
        assert self.is_element_value_equal_to('Please provide your zip code.', *self.lbl_zip_code_error_message) is True
        print('The correct zip code error message (Please provide your zip code.) is displayed')

    def validate_country_error_message(self):
        assert self.is_element_found(*self.lbl_country_error_message) is True
        assert self.is_element_value_equal_to('Please provide your country.', *self.lbl_country_error_message) is True
        print('The correct country error message (Please provide your country.) is displayed')
